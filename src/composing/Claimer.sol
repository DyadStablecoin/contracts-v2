...
contract Claimer is IClaimer, Owned {
  ...
  // set of dNFT ids that `claim` is going to be called for
  ...
  modifier onlyNftOwner(uint id) {
    if (dNft.ownerOf(id) != msg.sender) revert NotNFTOwner();
    _;
  }

  constructor(IDNft _dnft, Config memory _config) 
    Owned(msg.sender) 
  {
    dNft   = _dnft;
    config = _config;
  ...
  function _remove(uint id) 
    internal 
  {
    if (!claimers.remove(id)) revert IdNotInSet();
    emit Removed(id);
  }
  ...
  function _hasPermissions(uint id) 
    internal 
    view 
    returns (bool) 
  {
